...
public class Bow : MonoBehaviour
{
    ...
    void Update()
    {
        arrowCounter.text = bowSettings.arrowCount.ToString();
    }
    ...
    public void Fire(Vector3 hitPoint)
    {
        ...
        RaycastHit hit;
        if (Physics.Raycast(bowSettings.arrowPos.position, dir, out hit, 500f))
        {
            PhotonView targetPhotonView = hit.collider.gameObject.GetComponent<PhotonView>();
            if (targetPhotonView != null && targetPhotonView.IsMine)
            {
                targetPhotonView.RPC("TakeDamage", RpcTarget.All, 25f); // Adjust damage value as needed
            }
        }
    }
